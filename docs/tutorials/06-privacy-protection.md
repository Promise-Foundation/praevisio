...
If you want a ready-made repo with redaction tests wired in, use the lab case:

```
cd ~/projects/praevisio-lab
python -m praevisio_lab run-case privacy_redaction   --registry cases/manifest.yaml   --mode praevisio   --workspace .praevisio-lab/work   --bundles-dir .praevisio-lab/bundles   --json
```
...
def _redact_credit_card(match: re.Match[str]) -> str:
    return "[REDACTED_CREDIT_CARD]" if _luhn_is_valid(match.group(0)) else match.group(0)
